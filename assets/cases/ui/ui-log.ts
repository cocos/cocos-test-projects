import { _decorator, Node, Size, Component, Prefab, instantiate, UITransformComponent, WidgetComponent, fragmentText, safeMeasureText, LabelComponent, ScrollViewComponent, Vec3, LayoutComponent } from "cc";
...
export class UILog extends Component {
    ...
    set size(value){
        ...
        if(this.logPanel){
            const transform = this.logPanel.getComponent(UITransformComponent);
    ...
    private _scrollView: ScrollViewComponent = null;
    private _offset = new Vec3();
    private _originPos = new Vec3();
    private _layout: LayoutComponent = null;
    ...
    public initLog(config: ILogConfig) {
        ...
        const scrollView = panel.getComponent(ScrollViewComponent);
        this._scrollView = scrollView;
        this._layout = scrollView.content.getComponent(LayoutComponent);
        this._originPos.set(scrollView.content.position);
        this._originPos.y = this._size.height / 2;
        const transform = panel.getComponent(UITransformComponent);
        ...
        if (!!config.isAlign) {
            const widget = panel.addComponent(WidgetComponent);
        ...
        const maskWidget = mask.addComponent(WidgetComponent);
    ...
    public addLabel(str: string){
        ...
        item.parent = content;
        const itemTransComp = item.getComponent(UITransformComponent);
        const itemBgTransComp = item.children[0].getComponent(UITransformComponent);
        ...
        const labelTransComp = item.children[1].getComponent(UITransformComponent);
        labelTransComp.width = maxWidth - TOTAL_PADDING;
        labelTransComp.height = spliteStrings.length * FIXED_HEIGHT;
        const labelComp = labelTransComp.getComponent(LabelComponent);
        labelComp.string = text;
        this._layout.updateLayout();

        if (content.height > this._size.height) {
            this._offset.set(0, content.height - this._size.height, 0);
    ...
    private _allocItem(){
        ...
        const root = instantiate(this.item) as Node;
        return root;
    }
